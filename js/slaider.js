$(document).ready(function () {
    ...
    //Получаем массив ссылок из навигации слайдера
    $(".contentslider").css({
    ...
    // Дописал для кнопок вперед назад
    
    $("#back_slider").click(function () {
        $active = $(".active");
        
        var slidenumber = $active.attr("rel");
        if (slidenumber == 1) {
            slidenumber = $contentnav.length-1;
        } else {
            slidenumber = slidenumber - 2;
        }

        var slideposition = slidenumber * contentwidth;
        $(".contentnav a").removeClass('active');

        $contentnav.eq(slidenumber).addClass('active');
        clearInterval(play);
        $(".contentslider").animate({
            left: -slideposition
        }, 500);
    ...
    $("#forward_slider").click(function () {
    $active = $(".active");
    

    var slidenumber = $active.attr("rel");
    if (slidenumber == $contentnav.length) {
        slidenumber = 0;
    } 
